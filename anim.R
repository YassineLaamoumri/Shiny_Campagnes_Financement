...
data <- read.csv("projet/data_ulule_2019.csv",row.names = 1)
...
data %>% group_by(category) %>% summarise(mean_amount =mean(amount_raised)) %>% ggplot(aes(x=category,y=mean_amount,fill=category)) +geom_bar(stat = 'identity') + theme_minimal() +
  labs(title =
         'Montant moyen des campagnes financées par catégorie', x = 'Catégorie', y = 'Montant moyen')
